"""Checkpoint saver (SQLite): unit behavior + engine point-in-time rewind."""
from portfoliopilot.simulation.checkpoint import CheckpointSaver, SqliteSaver
from portfoliopilot.simulation.engine import SimulationEngine


def test_checkpoint_saver_is_sqlite_backed(tmp_path):
    cp = SqliteSaver(tmp_path / "checkpoints.sqlite")
    cp.save(0, {"month_index": 1, "current_date": "2020-01-31",
                "portfolio_value": 100000, "simulation_id": "sim_x"})
    assert (tmp_path / "checkpoints.sqlite").exists()
    # alias points at the SQLite implementation
    assert CheckpointSaver is SqliteSaver
    cp.close()
