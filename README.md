...
- **Checkpoints.** A point-in-time checkpoint is saved after every completed
  month (`data/checkpoints/`); the engine can `resume_latest()` or `restore_to(month)`
  (rewind), exposed via `/api/checkpoints`, `/api/resume`, `/api/restore`.
