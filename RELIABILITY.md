...
`simulation/checkpoint.py` provides a SQLite-backed `SqliteSaver` (consistent
with the monitor and memory stores, and with LangGraph's checkpointer
convention). It writes a checkpoint row after **every completed month** to
`data/checkpoints.sqlite` (table `checkpoints`, keyed by `completed_month`).
Heavier monitor / memory state already lives in their own SQLite databases.
